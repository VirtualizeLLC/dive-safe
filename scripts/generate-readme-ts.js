...
function htmlTableToMarkdown(html) {
	...
	return html.replace(/<table>[\s\S]*?<thead>([\s\S]*?)<\/thead>[\s\S]*?<tbody>([\s\S]*?)<\/tbody>[\s\S]*?<\/table>/gi, (match, thead, tbody) => {
		// Parse headers
		const headerRow = /<tr>([\s\S]*?)<\/tr>/i.exec(thead);
		const headers = headerRow ? Array.from(headerRow[1].matchAll(/<th[^>]*>([\s\S]*?)<\/th>/gi)).map(m => m[1].trim().replace(/\n/g, ' ')) : [];
		// Parse body rows
		const bodyRows = Array.from(tbody.matchAll(/<tr>([\s\S]*?)<\/tr>/gi)).map(rowMatch => {
			return Array.from(rowMatch[1].matchAll(/<td[^>]*>([\s\S]*?)<\/td>/gi)).map(m => m[1].trim().replace(/\n/g, ' '));
		});
		// Build markdown
		let md = '| ' + headers.join(' | ') + ' |\n';
		md += '| ' + headers.map(() => '---').join(' | ') + ' |\n';
