...
async function main() {
	...
	if (!token) {
		console.error(
			'Missing GitHub token. Pass --token or set GITHUB_TOKEN env var.',
		)
		process.exit(2)
	}
	...
	console.log(
		`Using owner=${owner} repo=${repo} tag=${tag} file=${absoluteApk}`,
	)
	...
	try {
		const out = run(
			`curl -s -H "Authorization: token ${token}" https://api.github.com/repos/${owner}/${repo}/releases/tags/${encodeURIComponent(tag)}`,
		)
	...
	if (!release) {
		...
		const out = run(
			`curl -s -X POST -H "Authorization: token ${token}" -H "Content-Type: application/json" -d '${data}' https://api.github.com/repos/${owner}/${repo}/releases`,
		)
		release = JSON.parse(out)
	}
	...
	const uploadUrlTemplate =
		release.upload_url || release.upload_url_template || null

	// 3) If an asset with same name exists, delete it
	try {
		const assetsOut = run(
			`curl -s -H "Authorization: token ${token}" https://api.github.com/repos/${owner}/${repo}/releases/${releaseId}/assets`,
		)
		...
		if (Array.isArray(assets)) {
			...
			if (existing) {
				console.log(
					'Found existing asset with same name; deleting asset id',
					existing.id,
				)
				run(
					`curl -s -X DELETE -H "Authorization: token ${token}" https://api.github.com/repos/${owner}/${repo}/releases/assets/${existing.id}`,
				)
			}
		}
	} catch (e) {
	...
	// 4) Upload asset
	const uploadBase =
		(uploadUrlTemplate && uploadUrlTemplate.split('{')[0]) ||
	...
	try {
		...
		run(
			`curl -sS -X POST -H "Authorization: token ${token}" -H "Content-Type: ${contentType}" --data-binary @${absoluteApk} "${uploadUrl}"`,
		)
